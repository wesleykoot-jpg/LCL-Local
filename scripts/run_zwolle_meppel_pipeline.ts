 ...
 *
 * Usage:
 *   deno run --allow-net --allow-env --allow-read scripts/run_zwolle_meppel_pipeline.ts
 *
 * Required env vars:
...
// Sources include municipal agendas, regional event sites, venue listings
...
const ZWOLLE_MEPPEL_SOURCES: EventSource[] = [
  ...
  {
    id: "zwolle-sassenpoort",
    name: "Museum de Sassenpoort",
    url: "https://www.museumdesassenpoort.nl/agenda",
    city: "Zwolle",
    lat: 52.5123,
    lng: 6.0901,
    category: "culture",
    description: "Historical museum events",
  ...
  {
    id: "zwolle-vrijemarkt",
    name: "Zwolle Vrije Markt",
    url: "https://www.zwolsevrijemarkt.nl",
    city: "Zwolle",
    lat: 52.5168,
    lng: 6.083,
    category: "market",
    description: "Local markets and fairs",
    selectors: [".markt-item", ".event", "[class*='markt']"],
  },
  {
...
async function main() {
  ...
  for (const source of ZWOLLE_MEPPEL_SOURCES) {
    ...
    if (existing) {
      ...
      if (updateError) {
        console.log(`  ⚠️  ${source.name}: Update failed - ${updateError.message}`);
      } else {
      ...
      if (insertError) {
        console.log(`  ⚠️  ${source.name}: Insert failed - ${insertError.message}`);
      } else {
  ...
  // Track scrape results per source
  const scrapeResults: Record<string, { staged: number; errors: string[] }> =
    {};
  ...
  for (const sourceId of configuredSourceIds) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        scrapeResults[sourceId] = { staged, errors: [] };
        console.log(`✅ ${staged} events staged`);
      } else {
        const errorText = await response.text();
        scrapeResults[sourceId] = { staged: 0, errors: [errorText] };
        console.log(`⚠️  Error: ${response.status}`);
      }
    } catch (e) {
      scrapeResults[sourceId] = { staged: 0, errors: [String(e)] };
  ...
  (eventsBySource || []).forEach((e) => {
    ...
    sourceStats[sid].categories[cat] = (sourceStats[sid].categories[cat] || 0) + 1;
    ...
    .forEach(([method, count]) => {
      ...
      const pct =
        stagingByMethod && stagingByMethod.length > 0
          ...
          : "0";
      console.log(`     ${method.padEnd(22)} │ ${String(count).padStart(5)} │ ${pct.padStart(5)}% │ ${desc}`);
    });
  ...
  const sortedSources = Object.entries(sourceStats).sort(([idA, a], [idB, b]) => {
  ...
  if ((finalEventCount || 0) >= 1800) {
    ...
    console.log("  ⚠️  PARTIAL: Moderate event count, some sources may need attention");
  } else {
