...
import { toPostgisPoint } from '@/shared/lib/postgis';
...
describe('Data Flow Dry Run: Serper → DB → EventCard', () => {
  ...
  const meppelCoords = { lat: meppel?.lat ?? 52.6957, lng: meppel?.lng ?? 6.1944 };
  ...
  it('maps Serper coordinates into PostGIS POINT and renders map marker in correct order', () => {
    ...
    expect(mapFrame.getAttribute('src')).toContain(`${meppelCoords.lat},${meppelCoords.lng}`);
  ...
  it('shows Location Unknown badge when coordinates are missing or zeroed', () => {
    ...
    expect(screen.getAllByText(/Location Unknown/i).length).toBeGreaterThanOrEqual(1);
