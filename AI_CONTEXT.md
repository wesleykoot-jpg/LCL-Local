...
local events globally, with an AI-powered event scraper.
...
- **Agent Workflow**:
  - **SQL Connection**: Verified connection to Supabase via SQLTools in
    `.vscode/settings.json`. Used for schema validation and data audits.
  - **Git Commits**: Authorized to perform commits directly with short
    descriptions upon task completion. (Ref:
    [.agent/workflows/commit.md](file:///Users/wesleykoot/Documents/Repo/LCL-Local/.agent/workflows/commit.md))
