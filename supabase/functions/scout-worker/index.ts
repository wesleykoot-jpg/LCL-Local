...
function extractCityName(
  ...
  const urlMatch = sourceUrl.match(
    /ontdek(\w+)|beleef(\w+)|visit(\w+)|uit(\w+)|(\w+)\.nl/i,
  );
  if (urlMatch) {
    const city =
      urlMatch[1] || urlMatch[2] || urlMatch[3] || urlMatch[4] || urlMatch[5];
...
async function scoutSource(
  ...
  try {
    ...
    const cityName =
      source.location_name || extractCityName(sourceName, source.url);
    ...
    const recipe = await generateExtractionRecipe(
      aiApiKey,
      html,
      sourceContext,
    );
    ...
    await logInfo(
      "scout-worker",
      "scoutSource",
      `Recipe generated for ${sourceName}`,
      {
        source_id: sourceId,
...
export const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    // Use OpenAI directly for scouting
    ...
    if (targetSourceId) {
      ...
      if (error) {
        ...
        return new Response(
          JSON.stringify({
            message: "Failed to claim sources",
            error: error.message,
          }),
          {
            status: 500,
    ...
    console.log(
      `[Scout] Completed: ${successCount} success, ${failCount} failed in ${totalTimeMs}ms`,
    );
