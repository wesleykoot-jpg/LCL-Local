...
const HYDRATION_PATTERNS = [
  {
    name: "__NEXT_DATA__",
    regex: /<script\s+id="__NEXT_DATA__"[^>]*>([\s\S]*?)<\/script>/i,
  },
  {
    name: "__NUXT__",
    regex: /window\.__NUXT__\s*=\s*(\{[\s\S]*?\});?\s*<\/script>/i,
  },
  {
    name: "__INITIAL_STATE__",
    regex: /window\.__INITIAL_STATE__\s*=\s*(\{[\s\S]*?\});?\s*<\/script>/i,
  },
  {
    name: "__PRELOADED_STATE__",
    regex: /window\.__PRELOADED_STATE__\s*=\s*(\{[\s\S]*?\});?\s*<\/script>/i,
  },
  {
    name: "__APP_DATA__",
    regex: /window\.__APP_DATA__\s*=\s*(\{[\s\S]*?\});?\s*<\/script>/i,
  },
];
...
function findEventsInObject(
  ...
  if (Array.isArray(obj)) {
    for (const item of obj) {
      if (isEventLike(item)) {
        const event = objectToRawEventCard(
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (
        EVENT_KEYWORDS.some((k) => keyLower.includes(k)) ||
        keyLower === "data" ||
        keyLower === "items"
      ) {
...
function isEventLike(obj: unknown): boolean {
  ...
  const hasTitle =
    "title" in o || "name" in o || "eventName" in o || "headline" in o;
  ...
  // Or a venue/location
  const hasLocation =
    "venue" in o || "location" in o || "place" in o || "address" in o;
  ...
  try {
    ...
    jsonLdScripts.each((_, script) => {
      ...
      for (const item of items) {
        ...
        const event = schemaToRawEventCard(
          item as Record<string, unknown>,
          ctx.baseUrl,
        );
        if (event) {
...
function schemaToRawEventCard(
  ...
  const title =
    getStringValue(schema["name"]) || getStringValue(schema["headline"]);
  if (!title) return null;
 ...
 */
function inferCategoryFromSchema(
  schema: Record<string, unknown>,
): string | undefined {
...
export function extractFromMicrodata(
  ...
  try {
    ...
    $(
      '[itemtype*="schema.org/Event"], [itemtype*="schema.org"][itemtype*="Event"]',
    ).each((_, el) => {
      const $el = $(el);
 ...
 */
export async function extractFromFeeds(
  html: string,
  ...
  try {
    ...
    $('a[href*=".ics"], a[href*=".ical"], a[href^="webcal://"]').each(
      (_, el) => {
        const href = $(el).attr("href");
        if (href)
          feedUrls.push(
            resolveUrl(href.replace("webcal://", "https://"), ctx.baseUrl),
          );
      },
    );
    ...
    if (
      feedUrls.length === 0 &&
      (ctx.feedDiscovery || ctx.preferredMethod === "feed")
    ) {
      try {
    ...
    return {
      ...
      found: events.length,
      error:
        events.length === 0 && feedUrls.length > 0
  ...
  if (events.length === 0) {
    $("entry").each((_, entry) => {
      ...
      events.push({
        ...
        location: "",
        description:
          $entry.find("summary, content").first().text().trim() || "",
        detailUrl:
...
export function parseIcsFeed(
  ...
  const eventBlocks =
    icsContent.match(/BEGIN:VEVENT[\s\S]*?END:VEVENT/gi) || [];
...
const DEFAULT_SELECTORS = [
  ...
  ".agenda-item",
  ".calendar-event",
  // Removed risky catch-all selectors like [class*='event'] that match body/main
  "li.event",
  ".post-item",
  ".datum-item",
  ".activity-card",
  ...
  try {
    ...
    for (const selector of selectors) {
      ...
      elements.each((_, el) => {
        ...
        // Safety check: specific event cards shouldn't be massive
        ...
        let dateText =
          $el
            .find(
              "time, .date, [class*='date'], [class*='datum'], [class*='tijd'], [datetime]",
            )
            .first()
            .text()
            .trim() ||
        ...
        if (!dateText) {
          ...
          const datePattern =
            /(?:\d{1,2}\s+(?:jan|feb|mar|mrt|apr|may|mei|mai|mär|jun|jul|aug|sep|oct|okt|nov|dec|dez)[a-z]*|\d{1,2}[-/.]\d{1,2}[-/.]\d{2,4})/i;
        ...
        // Extract location
        const location = $el
          .find(".location, .venue, [class*='location'], [class*='venue']")
          .first()
          .text()
          .trim();
...
export async function runExtractionWaterfall(
  ...
  const trace: Record<
    ExtractionStrategy,
    Omit<ExtractionResult, "events">
  > = {} as Record<ExtractionStrategy, Omit<ExtractionResult, "events">>;
  ...
  const strategies: (ExtractionStrategy | "microdata")[] =
    ctx.preferredMethod && ctx.preferredMethod !== "auto"
      ? [
          ctx.preferredMethod,
          ...(
            ["hydration", "json_ld", "microdata", "feed", "dom"] as (
              | ExtractionStrategy
              | "microdata"
            )[]
