/*
  ...
  1. New Tables
    - `profiles`
      - User profiles with reliability scores and location data
      - Tracks events attended/committed, current persona, verified status
    
    - `persona_stats`
      - Per-persona statistics (family, gamer)
      - Tracks rallies hosted, newcomers welcomed, host rating
    
    - `persona_badges`
      - Badges earned by users for each persona
      - Includes badge name, level, icon, and earned date
    
    - `events`
      - All events (anchors, forks, signals)
      - Supports geospatial location, parent events, categories
      - Categories: cinema, crafts, sports, gaming, market
    
    - `event_attendees`
      - Many-to-many relationship between events and profiles
      - Tracks attendance status, ticket numbers, check-in status
  
  2. Security
    - Enable RLS on all tables
    - Anyone can view profiles, events, persona stats/badges, and attendees
    - Authenticated users can create profiles and events
